              ...
              <tbody>
                @for (slot of edt(); track slot.id) {
                  <tr>
                    ...
                    @if (canEdit()) { <td><button (click)="remove(slot.id)" class="btn btn-icon btn-danger" title="Supprimer"><span class="material-symbols-outlined" style="font-size:18px">delete</span></button></td> }
                  </tr>
                } @empty {
